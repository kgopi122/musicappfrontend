...
const SearchPage = () => {
  ...
  const toggleFavorite = async (song) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to toggle favorite for song ${song.title}`);
      }

      const result = await response.json();

      // Update UI
      const updatedSongs = searchResults.map(s => 
  ...
  const handleLikeSong = async (song) => {
    ...
    try {
      const userEmail = localStorage.getItem('userEmail');
      if (!userEmail) {
        showToast('Please log in to like songs', 'error');
        return;
      }
